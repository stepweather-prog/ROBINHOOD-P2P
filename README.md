...
│ │
│ craftArrow() │
│ генерирует PeerID, ECDH, nonce │
│ SHA(nonce+'beacon') → ключ маяка │
│ AES(inner, ключ) → шифрует эмодзи │
│ POST /beacon waiting_<peerIdA> │
│ │
│ joinBeacon(peerIdA) │
│ GET /beacon waiting_<peerIdA> │
│ получает маяк │
│ проверяет HMAC(nonce+peerId) │
│ SHA(nonce+'beacon') → ключ │
│ AES⁻¹(inner, ключ) → эмодзи │
│ │
│ verification-emoji ←─────────────── │
│ (через emoji_<peerIdA>) │
│ │
│ Пир А видит эмодзи │
│ Пир Б вводит эмодзи │
│ │
│ confirmVerification() │
│ ECDH → общий секрет │
│ SHA(секрет+эмодзи) → hash │
│ │
│ verification-ack ─────────────────→ │
│ beacon-response ──────────────────→ │
│ (через waiting_<peerIdB>) │
│ │
│ Канал открыт 🔓 │
│ Ratchet + AES-256-GCM │
│ WebRTC DataChannel (если доступен) │
...
- Максимальная длительность: 120 секунд
...
``json
{
  ...
  "nonce": "<128 bit hex>",
  "targetPeerId": "<32 hex chars>",
  "sig": "<HMAC-SHA256, base64>"
}
inner (расшифрованный)
{
  "timestamp": "<unix ms>",
  "peerId": "<32 hex chars>",
  "emoji": ["😀","😂","🤣","😍","😘"]
...
// Отправить голосовое
await P2PPong.sendVoiceMessage(channelId, audioBase64, durationSec)
...
Макс. длительность голосового	120 сек
Ответственность
