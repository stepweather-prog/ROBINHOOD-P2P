...
const P2PPong = {
    ...
    _webRTC: {}, _webRTCPolling: {}, _msgPollTimers: {}, _msgReadTimers: {}, _msgDeleteQueue: [],
    ...
    _startMsgPoll(chId) { if (this._msgPollTimers[chId]) return; log('_startMsgPoll', chId); const poll = () => { if (!this._channels[chId]) { this._stopMsgPoll(chId); return; }
        this._get('/beacon?key=msg_' + chId).then(d => { if (d?.packet) { this._handleIn(d.packet, chId); this._scheduleMsgDelete(chId); } this._msgPollTimers[chId] = setTimeout(poll, 5000); }).catch(() => { this._msgPollTimers[chId] = setTimeout(poll, 5000); }); }; poll(); },
    ...
    _scheduleMsgDelete(chId) { if (this._msgReadTimers[chId]) clearTimeout(this._msgReadTimers[chId]); const msgId = chId + '_' + Date.now(); this._msgDeleteQueue.push(msgId); this._msgReadTimers[chId] = setTimeout(() => { this._post('/beacon', { keyHash: 'msg_' + chId, packet: '' }); this._msgDeleteQueue = this._msgDeleteQueue.filter(id => id !== msgId); log('msg deleted', chId); }, 60000); },
    ...
    async _handleIn(blobData, chId) { let d; try { d = JSON.parse(blobData); } catch(e) { return; }
        ...
        if (d?.type === 'beacon' && d.pubKey && d.inner) { if (d.targetPeerId && d.targetPeerId !== this._peerId) return; if (d.sig && !await verifyHMAC(JSON.stringify(d), d.sig, await SHA('beacon'))) return; log('beacon received');
            this._emit('beacon-received', { peerId: d.peerId, accept: async () => { const rpk = await importPublicKey(d.pubKey); const kp = await generateKeyPair(); const mpk = await exportPublicKey(kp); const ss = await deriveSecret(kp, rpk); const nid = RND();
                ...
                this._stats.channelsOpened++; await this._saveCh(); this._emit('channel-opened', { channelId: nid, peerId: d.peerId, nick: 'Лучник', avatar: '001' }); this._startMsgPoll(nid); this._pendingWebRTC.set(nid, true); } }); return; }
        if (d?.type === 'beacon-response' && d.pubKey && d.inner) { log('beacon-response received'); for (const [bid, b] of Object.entries(this._beacons)) { if (!b.beaconKey) continue; const dec = await decryptAES(d.inner, b.beaconKey); if (!dec) continue; let p; try { p = JSON.parse(dec); } catch(e) { continue; } if (p.nonce !== b.nonce) continue;
            ...
            delete this._beacons[bid]; this._stopPolling(); this._stats.channelsOpened++; await this._saveCh(); this._emit('channel-opened', { channelId: nid, peerId: d.peerId, nick: 'Лучник', avatar: '001' }); this._startMsgPoll(nid); this._pendingWebRTC.set(nid, true); if (this._verificationConfirmed) { this.startWebRTC(nid); this._pendingWebRTC.set(nid, false); } return; } }
