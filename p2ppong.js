// ===================================================================
// P2PPong — Рабочий. Первое касание.
// ===================================================================
...
const P2PPong = {
    ...
    _stats: { messagesSent: 0, messagesReceived: 0, channelsOpened: 0 },
    _housekeepInterval: null,
    _pollTimer: null, _pollStart: null, _pollKey: null, _pollMax: 150,
    _webRTC: {}, _webRTCPolling: {}, _msgPollTimers: {},
    _pendingVerification: null, _verificationEmoji: null, _crafting: false,
    _dedupTimers: {},
    ...
    async craftArrow() {
        ...
        try {
            ...
            const inner = await encryptAES(JSON.stringify({ peerId: this._peerId, emoji: correctEmoji }), await SHA('beacon'));
            const bd = { type: 'beacon', pubKey: pk, peerId: this._peerId, inner };
    ...
    async confirmVerification() {
        ...
        const chId = RND();
        this._channels[chId] = { secret: ss, ratchetKey: ss, ratchetIndex: 0, oldKeys: [], peerId: bd.peerId, blobs: [], expires: Date.now() + 600000, createdAt: Date.now(), verificationHash };
        ...
        const ok = await this._post('/beacon', { keyHash: 'waiting_' + targetPeerId, packet: JSON.stringify({ type: 'beacon-response', pubKey: mpk, peerId: this._peerId, inner: bd.inner, channelId: chId, verificationHash }) });
    ...
    async _post(path, body) { for (const s of this._signalServers) { try { const r = await fetch(s.url + path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: AbortSignal.timeout(5000) }); if (r.ok) return r.json(); } catch(e) {} } return null; },
    ...
    async startWebRTC(chId, asInitiator) {
        var ch = this._channels[chId]; if (!ch || this._webRTC[chId]) return;
        var pc = new RTCPeerConnection({ iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] });
        this._webRTC[chId] = { pc: pc, dc: null, iceBuffer: [], connected: false, initiator: asInitiator, seenMessages: new Set() };
        var me = this;
        if (asInitiator) {
            var dc = pc.createDataChannel('chat'); me._webRTC[chId].dc = dc;
            dc.onopen = function() { me._webRTC[chId].connected = true; me._emit('peer-connected', { channelId: chId }); me._stopWebRTCPoll(chId); };
            ...
            pc.ondatachannel = function(e) { var dc = e.channel; me._webRTC[chId].dc = dc;
                dc.onopen = function() { me._webRTC[chId].connected = true; me._emit('peer-connected', { channelId: chId }); me._stopWebRTCPoll(chId); };
        ...
        if (asInitiator && !me._webRTC[chId].offerSent) { var offer = await pc.createOffer(); await pc.setLocalDescription(offer); me._webRTC[chId].offerSent = true; me._sendWSig(chId, { type: 'webrtc-offer', sdp: JSON.stringify(pc.localDescription) }); }
    ...
    async _handleIn(blobData, chId) {
        ...
        if (d.type === 'beacon-response' && d.pubKey && d.inner) { var keys = Object.keys(this._beacons); for (var i = 0; i < keys.length; i++) { var b = this._beacons[keys[i]]; if (!b.keyPair) continue; var rpk = await importPublicKey(d.pubKey); var ss = await deriveSecret(b.keyPair, rpk); var nid = d.channelId || RND(); if (d.verificationHash) { var expectedHash = await SHA(ss + (this._verificationEmoji ? this._verificationEmoji.join('') : '')); if (d.verificationHash !== expectedHash) { log('_handleIn', 'HASH MISMATCH'); return; } } this._channels[nid] = { secret: ss, ratchetKey: ss, ratchetIndex: 0, oldKeys: [], peerId: d.peerId, blobs: [], expires: Date.now() + 600000, createdAt: Date.now(), verificationHash: d.verificationHash }; delete this._beacons[keys[i]]; this._stopPolling(); this._stats.channelsOpened++; this._emit('channel-opened', { channelId: nid, peerId: d.peerId, nick: 'Лучник', avatar: '001' }); this._startMsgPoll(nid); this.startWebRTC(nid, false); return; } }
        var ch = this._channels[chId]; if (ch && ch.ratchetKey) { var u = await unpackBlob(blobData, ch); if (u) { var dedupKey = chId + '_' + (u.n || u._t || ''); if (this._dedupTimers[dedupKey]) return; this._dedupTimers[dedupKey] = setTimeout(function() { delete me._dedupTimers[dedupKey]; }, 300000); ch.blobs.push({ d: u.d || u.text || '', t: u._t || Date.now(), n: u.n || '', from: 'them' }); ch.expires = Date.now() + 600000; this._stats.messagesReceived++; this._emit('message-received', { channelId: chId, text: u.d || u.text || '', from: 'them', timestamp: u._t || Date.now() }); } }
    },

    getStats() { return { peerId: this._peerId, state: this._state, channels: Object.keys(this._channels).length }; },
    async destroy() { this._stopPolling(); Object.keys(this._msgPollTimers).forEach(function(id) { clearTimeout(this._msgPollTimers[id]); }); this._msgPollTimers = {}; Object.keys(this._webRTCPolling).forEach(function(id) { clearTimeout(this._webRTCPolling[id]); }); this._webRTCPolling = {}; Object.keys(this._webRTC).forEach(function(id) { try { this._webRTC[id].pc.close(); } catch(e) {} }); this._webRTC = {}; if (this._housekeepInterval) clearInterval(this._housekeepInterval); this._channels = {}; this._beacons = {}; this._listeners = {}; this._state = 'idle'; this._peerId = null; this._pendingVerification = null; this._emit('destroyed'); }
};
...
async function unpackBlob(blob, ch) { var dec = await decryptAES(blob, ch.secret); if (!dec) return null; var result = await tryDecryptWithKey(dec, ch.ratchetKey || ch.secret); if (result) return result; if (ch.oldKeys) { for (var i = ch.oldKeys.length - 1; i >= 0; i--) { result = await tryDecryptWithKey(dec, ch.oldKeys[i].key); if (result) return result; } } return null; }
