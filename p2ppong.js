...
const P2PPong = {
        ...
        if (!d?.packet) {
            for (const s of this._signalServers.filter(s=>s.type==='http')) {
                try {
                    ...
                    if (r.ok) { const data = await r.json(); if (data?.status==='found'&&data.packet) { d = data; log('beacon-found',s.name); break; } }
                } catch(e) {}
            }
        }
        if (!d?.packet) { this._emit('error', { message: 'Метка не найдена' }); return false; }
        let bd;
        try { bd = JSON.parse(d.packet); } catch(e) { this._emit('error', { message: 'Метка повреждена' }); return false; }
    ...
    confirmVerification() {
        if (this._pending?.type==='joiner'&&this._beaconId&&this._verificationCode) {
            this._post('/beacon',{keyHash:'code_'+this._beaconId,packet:JSON.stringify({type:'verification-code',code:this._verificationCode,peerId:this._peerId,pubKey:this._kp.publicKey})});
        }
    ...
    async sendMessage(chId, text) {
        ...
        if (!ch) return false;
        const nonce=RND();
        const md=JSON.stringify({type:'text',d:text,t:Date.now(),n:nonce,from:this._peerId,nick:this._myNick,avatar:this._myAvatar});
    ...
    async _sendEncrypted(ch, chId, messageData, displayText, nonce) {
        ...
        if (ch.dhSendCount>=CONFIG.DH_RATCHET_THRESHOLD&&ch.dhKeyPair&&ch.dhRemotePubKey) {
            const dr=await this._dhRatchetStep(ch);
            if (dr) {
                const parsed=JSON.parse(messageData);
                parsed.dhPubKey=dr.newPubKey;
        ...
        try {
            ...
            if (sig.type==='webrtc-offer') {
                pc.setRemoteDescription(new RTCSessionDescription(JSON.parse(sig.sdp))).then(()=>{
                    ...
                    rtc.iceBuffer=[];
                    if (!rtc.initiator) {
                        pc.createAnswer().then(a=>pc.setLocalDescription(a)).then(()=>this._sendWSig(chId,{type:'webrtc-answer',sdp:JSON.stringify(pc.localDescription)}));
                    }
                });
                return;
            }
