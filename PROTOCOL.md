...
## 8. Адресная доставка маяков

### 8.1 Проблема broadcast

В ранних версиях сигнальный сервер рассылал маяки всем подключённым пирам. Это позволяло любому пиру перехватить маяк, предназначенный другому.

### 8.2 Решение: targetPeerId

Каждый маяк содержит поле targetPeerId. Сервер:
1. Принимает маяк
2. Проверяет наличие targetPeerId среди активных WebSocket-соединений
3. Если целевой пир онлайн — доставляет маяк только ему
4. Если целевой пир офлайн — маяк не доставляется никому

Никакого broadcast. Никакой утечки метаданных.
