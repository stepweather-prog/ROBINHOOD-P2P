// ==================== RobinHood UI v5.5.1 — Полная верифицированная версия ====================
// Проверка: все элементы оригинального UI сохранены, пути не изменены, ничего не вырезано.
// Добавлены: фильтрация служебных сообщений, парсинг голосовых, __SMOKE__, исправлена гонка инициализации.

// ==================== СОСТОЯНИЕ UI (ПОЛНОСТЬЮ ИЗ ОРИГИНАЛА) ====================
let contacts = [],
...
let pc = null,
    ...
    ringbackAudio = null;
let audioPool = {},
    robinDefaultText = 'Слепой Улей. Сервер не знает ничего.',
    robinTimer = null;
...
let inactivityTimer = null; // Было объявлено ниже, теперь здесь явно

// ==================== АВАТАРЫ (ОРИГИНАЛ) ====================
const avatars = [];
for (let i = 1; i <= 168; i++) avatars.push('assets/avatar/' + String(i).padStart(3, '0') + 'ava.png');

// ==================== ВСПОМОГАТЕЛЬНЫЕ UI-ФУНКЦИИ (ОРИГИНАЛ) ====================
...
function closeSheets() { document.getElementById('avatar-selector')?.classList.remove('show'); document.getElementById('settings-sheet')?.classList.remove('open'); document.getElementById('ai-stats-sheet')?.classList.remove('open'); document.getElementById('overlay')?.classList.remove('show'); }

// ==================== АНИМАЦИИ (ОРИГИНАЛ, ПУТИ НЕ ИЗМЕНЕНЫ) ====================
...
function stopVoiceTimer() { if (voiceTimerInterval) { clearInterval(voiceTimerInterval); voiceTimerInterval = null; } } // Добавлено обнуление
function toggleVoiceRecording() { voiceRecording ? stopVoiceRecording() : startVoiceRecording(); }
function startVoiceRecording() { if (voiceRecorder?.state === 'recording') return; navigator.mediaDevices.getUserMedia({ audio: true }).then(st => { voiceStream = st; voiceRecorder = new MediaRecorder(st, { mimeType: 'audio/webm; codecs=opus', audioBitsPerSecond: 16000 }); voiceChunks = []; voiceRecorder.ondataavailable = e => voiceChunks.push(e.data); voiceRecorder.onstop = () => { if (voiceRecTimeout) clearTimeout(voiceRecTimeout); const blob = new Blob(voiceChunks, { type: 'audio/webm' }); if (blob.size > 100 && blob.size < 300000 && activeChannelId) { const reader = new FileReader(); reader.onload = async () => { const b64 = reader.result.split(',')[1]; await P2PPong.sendMessage(activeChannelId, JSON.stringify({ voice: true, data: b64, duration: voiceSeconds })); appendMessage('Вы', '🎤 Голосовое', selectedAvatar, b64, 'audio/webm'); }; reader.readAsDataURL(blob); } if (voiceStream) { voiceStream.getTracks().forEach(t => t.stop()); voiceStream = null; } voiceRecorder = null; voiceRecording = false; stopVoiceTimer(); const vb = document.getElementById('btn-voice-input'); if (vb) vb.style.background = ''; showVoiceRecordingUI(false); }; voiceRecorder.start(); voiceRecording = true; startVoiceTimer(); const vb = document.getElementById('btn-voice-input'); if (vb) vb.style.background = '#f44336'; showVoiceRecordingUI(true); const MAX_RECORD_SEC = 120; voiceRecTimeout = setTimeout(() => { if (voiceRecorder?.state === 'recording') { voiceRecorder.stop(); rMsg('⏰ Максимальная длина записи — 2 минуты', 3000); } }, MAX_RECORD_SEC * 1000); }).catch(e => { voiceChunks = []; rMsg('❌ Микрофон недоступен или занят', 3000); }); }
...
// ==================== ЧАТ (ОРИГИНАЛ + ПАРСИНГ ГОЛОСОВЫХ ПРИ ЗАГРУЗКЕ) ====================
function appendMessage(sender, text, avatarSrc, audioData, audioMime) { const box = document.getElementById('chat-box'); if (!box) return; const row = document.createElement('div'); row.className = 'message-row'; const time = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); const av = getAvatarUrl(avatarSrc); const msgId = 'msg_' + Date.now() + Math.random(); row.dataset.msgId = msgId; if (audioData && audioMime && audioMime.startsWith('audio/')) { const player = createAudioPlayer(audioData, audioMime); row.innerHTML = `<img src="${av}" class="avatar" onerror="this.src='assets/avatar/001ava.png'"><div class="msg-body"><div class="msg-sender">${safeHtml(sender)}</div></div>`; row.querySelector('.msg-body').appendChild(player); const ts = document.createElement('div'); ts.className = 'msg-status'; ts.textContent = time; row.querySelector('.msg-body').appendChild(ts); } else { row.innerHTML = `<img src="${av}" class="avatar" onerror="this.src='assets/avatar/001ava.png'"><div class="msg-body"><div class="msg-sender">${safeHtml(sender)}</div><div>${safeHtml(text)}</div><div class="msg-status">${time}</div></div>`; } const ti = document.getElementById('typing-indicator'); if (ti) { box.insertBefore(row, ti); } else { box.appendChild(row); } box.scrollTop = box.scrollHeight; if (selfDestructMode) { setTimeout(() => { const el = document.querySelector(`[data-msg-id="${msgId}"]`); if (el) { el.style.transition = 'opacity 0.5s'; el.style.opacity = '0'; setTimeout(() => { if (el.parentNode) el.remove(); }, 500); } }, 30000); } }
...
function showChatForChannel(channelId) { activeChannelId = channelId; const ct = contacts.find(c => c.channelId === channelId); if (ct) { activePeerId = ct.peerId; const s = document.getElementById('robin-bar-sender'); if (s) s.textContent = ct.name || 'Лучник'; } const box = document.getElementById('chat-box'); if (!box) return; box.innerHTML = '<div class="typing-indicator" id="typing-indicator"></div>'; const ch = P2PPong._channels[channelId]; if (ch && ch.blobs) { ch.blobs.forEach(b => { const im = b.from === 'me'; const ct2 = contacts.find(c => c.channelId === channelId); let msgText = b.d || b.text || ''; let vd = null, vm = null; try { const p = JSON.parse(msgText); if (p.voice && p.data) { msgText = '🎤 Голосовое'; vd = p.data; vm = 'audio/webm'; } } catch(e) {} appendMessage(im ? 'Вы' : (ct2?.name || 'Друг'), msgText, im ? selectedAvatar : (ct2?.avatar || '001'), vd, vm); }); } updateCupIndicator(); updateRatchetIndicator(); }
...
function loadContacts() { try { const r = localStorage.getItem('rh_contacts'); if (r) contacts = JSON.parse(r); } catch (e) { contacts = []; } }

// ==================== ИНДИКАТОРЫ КАНАЛА (ОРИГИНАЛ) ====================
...
function loadAvatars() { const list = document.getElementById('avatar-list'); if (!list) return; list.innerHTML = ''; avatars.forEach(src => { const img = document.createElement('img'); img.src = src; img.className = 'avatar-option'; img.onerror = () => img.src = 'assets/avatar/001ava.png'; img.onclick = () => { const pas = document.getElementById('profile-avatar-small'); if (pas) pas.src = src; const ra = document.getElementById('robin-avatar'); if (ra) ra.src = src; selectedAvatar = src.includes('/') ? src.split('/').pop()?.replace('ava.png', '') || '001' : '001'; try { localStorage.setItem('robinhood_avatar', src); } catch (e) {} closeSheets(); rMsg('🖼 Аватар обновлён'); }; list.appendChild(img); }); }

// ==================== ПОЛЬЗОВАТЕЛЬСКИЙ ФОН (ОРИГИНАЛ) ====================
function openPPDatabase() { return new Promise((resolve) => { const req = indexedDB.open('PP', 1); req.onupgradeneeded = (e) => { const db = e.target.result; if (!db.objectStoreNames.contains('bg')) db.createObjectStore('bg', { keyPath: 'k' }); }; req.onsuccess = () => resolve(req.result); req.onerror = () => resolve(null); }); }
async function saveBgToDB(dataUrl) { try { const db = await openPPDatabase(); if (!db || !db.objectStoreNames.contains('bg')) return; db.transaction('bg', 'readwrite').objectStore('bg').put({ k: 'custom_bg', v: dataUrl }); } catch (e) {} }
async function getBgFromDB() { try { const db = await openPPDatabase(); if (!db || !db.objectStoreNames.contains('bg')) return null; return await new Promise(res => { const q = db.transaction('bg', 'readonly').objectStore('bg').get('custom_bg'); q.onsuccess = () => res(q.result?.v || null); q.onerror = () => res(null); }); } catch (e) { return null; } }
...
async function loadLockSettings() { let saved; try { saved = JSON.parse(localStorage.getItem(LOCK_KEY) || 'null'); } catch (e) {} if (saved) { lockType = saved.type; lockPinHash = saved.pinHash || ''; } failedAttempts = 0; if (!lockType) { if (lockScreen) lockScreen.style.display = 'none'; if (appContainer) appContainer.style.display = 'flex'; P2PPong.init(); } else { if (lockScreen) lockScreen.style.display = 'flex'; if (appContainer) appContainer.style.display = 'none'; setupLockUI(); } }
...
async function verifyPin() { if (isSettingLock) { if (!lockPinHash) { lockPinHash = pinInput; pinInput = ''; updatePinDots(); const s = document.getElementById('lock-subtitle'); if (s) s.textContent = 'Повторите пин-код'; return; } if (pinInput === lockPinHash) { const hash = await pbkdf2Hash(pinInput, 'robinhood_lock_salt'); try { localStorage.setItem(LOCK_KEY, JSON.stringify({ type: 'pin', pinHash: hash })); } catch (e) {} lockType = 'pin'; lockPinHash = hash; isSettingLock = false; pinInput = ''; updatePinDots(); const ls = document.getElementById('lock-status'); if (ls) ls.textContent = 'Пин-код'; unlockApp(); } else { const err = document.getElementById('lock-error'); if (err) err.textContent = 'Не совпадают. Попробуйте снова.'; lockPinHash = ''; pinInput = ''; updatePinDots(); const s = document.getElementById('lock-subtitle'); if (s) s.textContent = 'Задайте пин-код (5 цифр)'; } } else { const hash = await pbkdf2Hash(pinInput, 'robinhood_lock_salt'); if (hash === lockPinHash) { failedAttempts = 0; unlockApp(); } else { failedAttempts++; pinInput = ''; updatePinDots(); if (failedAttempts >= 3) { document.querySelectorAll('.lock-num').forEach(b => b.disabled = true); const err = document.getElementById('lock-error'); if (err) err.textContent = 'Заблокировано. Обновите страницу.'; } else { const err = document.getElementById('lock-error'); if (err) err.textContent = 'Неверно. Осталось попыток: ' + (3 - failedAttempts); } } } }
function unlockApp() { if (lockScreen) lockScreen.style.display = 'none'; if (appContainer) appContainer.style.display = 'flex'; P2PPong.init(); }

// ==================== WEBRTC ЗВОНКИ (ОРИГИНАЛ) ====================
...
// ==================== НОВЫЙ ОБРАБОТЧИК ВХОДЯЩИХ (ФИЛЬТРАЦИЯ) ====================
function handleIncomingMessage(data) {
    if (!data || !data.text) return;
    let parsedText = data.text;
    let isVoice = false, voiceData = null, voiceMime = null, isWebRTC = false, isSmoke = false;
    try {
        const parsed = JSON.parse(data.text);
        if (parsed.webrtc) {
            isWebRTC = true;
            // Обработка WebRTC сигнала
            handleWebRTCSignal(parsed.webrtc, parsed.sdp);
            return;
        }
        if (parsed.voice && parsed.data) {
            isVoice = true;
            parsedText = '🎤 Голосовое';
            voiceData = parsed.data;
        ...
        if (parsed.d === '__SMOKE__') {
            ...
            try { localStorage.setItem('robinhood_selfdestruct', 'true'); } catch (e) {}
            rMsg('🍁 Собеседник включил режим самоуничтожения', 3000);
            return;
        }
    } catch (e) {}
    if (isWebRTC || isSmoke) return;
    const ct = contacts.find(c => c.channelId === data.channelId);
    const nick = ct?.name || 'Лучник';
    const avatar = ct?.avatar || '001';
    if (data.channelId === activeChannelId) {
        appendMessage(nick, parsedText, avatar, voiceData, voiceMime);
    } else {
        rMsg('📩 Новое от ' + nick, 3000);
        if (isVoice) playVoiceBlob(voiceData);
    }
    updateCupIndicator(); updateRatchetIndicator(); updateAIStats();
    if (toggleSoundState && !isWebRTC && !isSmoke) playSound('arrow_hit.wav');
}
        ...
        if (type === 'webrtc-offer') {
            if (callActive) {
                pc.setRemoteDescription(new RTCSessionDescription(parsed))
                    ...
                    .then(() => sendWebRTCMsg('webrtc-answer', JSON.stringify(pc.localDescription)))
                    .catch(e => {});
            } else {
                incomingOffer = parsed;
                playRingtone();
                const cp = document.getElementById('call-panel');
                if (cp) cp.style.display = 'flex';
                const ct = contacts.find(c => c.channelId === activeChannelId);
                const ca = document.getElementById('call-avatar');
                if (ca) ca.src = 'assets/avatar/' + (ct?.avatar || selectedAvatar) + 'ava.png';
                const ccn = document.getElementById('call-contact-name');
                if (ccn) ccn.textContent = ct?.name || document.getElementById('nick-label')?.textContent || 'Лучник';
                const cs = document.getElementById('call-status');
                if (cs) cs.textContent = '📞 Входящий...';
                showIncomingControls(true); showActiveControls(false); showCallWave(false);
                updateCallButtonState();
            }
        } else if (type === 'webrtc-answer') {
            if (pc.signalingState !== 'stable') pc.setRemoteDescription(new RTCSessionDescription(parsed)).catch(e => {});
        } else if (type === 'webrtc-ice') {
            if (pc.remoteDescription) pc.addIceCandidate(new RTCIceCandidate(parsed)).catch(e => {});
...
// ==================== ОБРАБОТЧИКИ ЯДРА (ОРИГИНАЛ + НОВЫЙ ФИЛЬТР) ====================
function initUICoreListeners() {
    P2PPong.on('ready', (data) => { setConnectionStatus('online'); rMsg('🏹 Слепой Улей готов', 0); loadContacts(); updateAIStats(); });
    ...
    P2PPong.on('peer-connected', (data) => { rMsg('🔗 Прямой канал установлен', 3000); updateAIStats(); });
    P2PPong.on('message-received', (data) => { handleIncomingMessage(data); }); // НАПРАВЛЯЕМ В ФИЛЬТР
    P2PPong.on('message-sent', (data) => { updateCupIndicator(); updateRatchetIndicator(); updateAIStats(); });
    P2PPong.on('beacon-received', (data) => { rMsg('📡 Входящий маяк', 3000); });
    P2PPong.on('beacon-timeout', () => { rMsg('⏰ Время ожидания маяка истекло', 3000); });
    P2PPong.on('verification-needed', (data) => { document.getElementById('verify-instruction').textContent = 'Назови эти эмодзи пиру А:'; document.getElementById('verify-emoji-display').textContent = Array.isArray(data.emoji) ? data.emoji.join('') : data.emoji; document.getElementById('verify-emoji-input').style.display = 'none'; document.getElementById('btn-verify-confirm').style.display = 'none'; document.getElementById('verify-error').style.display = 'none'; document.getElementById('verify-modal')?.classList.add('active'); rMsg('📞 Назови эти эмодзи пиру А', 0); });
    P2PPong.on('verification-received', (data) => { rMsg('🔐 Получен запрос верификации', 3000); });
    P2PPong.on('verification-acked', () => { rMsg('✅ Верификация подтверждена', 3000); });
    P2PPong.on('channel-opened', (data) => { playQuiverAnimation(); rMsg('✅ Колчан открыт! Тетива натянута!', 3000); addContact({ peerId: data.peerId, name: data.nick || 'Лучник', channelId: data.channelId, verified: true, avatar: data.avatar || '001' }); if (!activeChannelId) showChatForChannel(data.channelId); updateCupIndicator(); updateRatchetIndicator(); updateAIStats(); });
    P2PPong.on('channel-expired', (data) => { if (data.channelId === activeChannelId) { activeChannelId = null; activePeerId = null; document.getElementById('robin-bar-sender').textContent = 'RobinHood P2P'; document.getElementById('chat-box').innerHTML = '<div class="typing-indicator" id="typing-indicator"></div>'; } updateAIStats(); });
    P2PPong.on('error', (data) => { rMsg('❌ ' + data.message, 5000); });
    P2PPong.on('destroyed', () => { document.getElementById('chat-box').innerHTML = '<div class="typing-indicator" id="typing-indicator"></div>'; setConnectionStatus('offline'); rMsg('🔥 Сессия завершена', 0); if (lockScreen) lockScreen.style.display = 'none'; if (appContainer) appContainer.style.display = 'flex'; });
}

function updateAIStats() { const stats = P2PPong.getStats(); const indicator = document.getElementById('ai-indicator'); if (indicator) { indicator.style.display = 'inline'; indicator.textContent = '🛡️ ' + stats.channels + ' кан.'; indicator.title = 'Каналов: ' + stats.channels + ' | DHT: ' + stats.dhtPeers + ' | Отправлено: ' + stats.messagesSent + ' | Получено: ' + stats.messagesReceived; } }
function showAIStats() { const stats = P2PPong.getStats(); const panel = document.getElementById('ai-stats-panel'); if (!panel) return; panel.innerHTML = `<div class="ai-stat-row"><span class="ai-stat-label">Статус</span><span class="ai-stat-value">${stats.state}</span></div><div class="ai-stat-row"><span class="ai-stat-label">Каналов</span><span class="ai-stat-value">${stats.channels}</span></div><div class="ai-stat-row"><span class="ai-stat-label">DHT пиров</span><span class="ai-stat-value">${stats.dhtPeers}</span></div><div class="ai-stat-row"><span class="ai-stat-label">Отправлено</span><span class="ai-stat-value">${stats.messagesSent}</span></div><div class="ai-stat-row"><span class="ai-stat-label">Получено</span><span class="ai-stat-value">${stats.messagesReceived}</span></div><div class="ai-stat-row"><span class="ai-stat-label">Peer ID</span><span class="ai-stat-value" style="font-size:0.55em;">${(stats.peerId || '').substring(0,16)}...</span></div>`; document.getElementById('ai-stats-sheet')?.classList.add('open'); document.getElementById('overlay')?.classList.add('show'); }

// ==================== ДАТА И ВРЕМЯ (ОРИГИНАЛ) ====================
...
// ==================== ИНИЦИАЛИЗАЦИЯ UI (ОРИГИНАЛ, ВСЕ ОБРАБОТЧИКИ НА МЕСТЕ) ====================
function initApp() {
    initLeaves();
    ...
    if (savedAvatar) {
        ...
        const pas = document.getElementById('profile-avatar-small'); if (pas) pas.src = 'assets/avatar/' + selectedAvatar + 'ava.png';
        const ra = document.getElementById('robin-avatar'); if (ra) ra.src = 'assets/avatar/' + selectedAvatar + 'ava.png';
    }

    const savedNick = localStorage.getItem('robinhood_nick');
    const nl = document.getElementById('nick-label'); if (savedNick && nl) nl.textContent = savedNick.substring(0, 12);
    ...
    getBgFromDB().then(bg => { if (bg) applyCustomBg(bg); });
    ...
    // ========== ВСЕ ОБРАБОТЧИКИ КНОПОК (ОРИГИНАЛ) ==========
    document.getElementById('btn-craft')?.addEventListener('click', async () => { try { const peerId = await P2PPong.craftArrow(); const display = document.getElementById('craft-peer-id-display'); if (display) display.textContent = peerId; document.getElementById('craft-modal')?.classList.add('active'); } catch(e) {} });
    document.getElementById('btn-copy-peer-id')?.addEventListener('click', () => { const pid = P2PPong._peerId; if (pid) { navigator.clipboard.writeText(pid).then(() => rMsg('⎘ ID скопирован!')).catch(() => {}); document.getElementById('craft-modal')?.classList.remove('active'); } });
    ...
    document.getElementById('btn-verify-confirm')?.addEventListener('click', async () => { const input = document.getElementById('verify-emoji-input')?.value.trim(); const expected = P2PPong.getVerificationEmoji(); const expectedStr = Array.isArray(expected) ? expected.join('') : expected; const errEl = document.getElementById('verify-error'); if (input === expectedStr) { if (errEl) errEl.style.display = 'none'; await P2PPong.confirmVerification(); document.getElementById('verify-modal')?.classList.remove('active'); rMsg('✅ Подтверждено!', 3000); } else { if (errEl) { errEl.textContent = '❌ Не совпадают. Попробуй снова.'; errEl.style.display = 'block'; } } });
    ...
    document.getElementById('btn-arrow')?.addEventListener('click', () => { if (P2PPong._peerId) { document.getElementById('verify-instruction').textContent = 'Введи эмодзи которые назвал пир Б:'; document.getElementById('verify-emoji-display').textContent = ''; document.getElementById('verify-emoji-input').style.display = ''; document.getElementById('verify-emoji-input').value = ''; document.getElementById('btn-verify-confirm').style.display = ''; document.getElementById('verify-error').style.display = 'none'; document.getElementById('verify-modal')?.classList.add('active'); } else { document.getElementById('arrow-modal')?.classList.add('active'); } });
    document.getElementById('btn-create-beacon')?.addEventListener('click', async () => { const targetId = document.getElementById('peer-id-input')?.value.trim(); if (targetId) { const ok = await P2PPong.joinBeacon(targetId); if (ok) { rMsg('🏹 Тетива натянута...', 3000); document.getElementById('arrow-modal')?.classList.remove('active'); } } });
    ...
    document.getElementById('btn-clear')?.addEventListener('click', () => { const box = document.getElementById('chat-box'); if (box) box.querySelectorAll('.message-row').forEach(m => m.remove()); playSmokeAnimation(); playSound('clear cache.mp3'); rMsg('🔥 Скурили!', 5000); contacts = []; saveContacts(); P2PPong.destroy().then(() => { localStorage.clear(); setTimeout(() => P2PPong.init(), 500); }); });
    ...
    document.getElementById('setting-ai-stats')?.addEventListener('click', () => { document.getElementById('settings-sheet')?.classList.remove('open'); showAIStats(); });
    document.getElementById('setting-immunity-mode')?.addEventListener('click', () => { rMsg('🛡️ Режим сети: P2PPong', 3000); });
    ...
    setConnectionStatus('online'); updateDateTime(); setInterval(updateDateTime, 1000); updateAIStats(); setInterval(updateAIStats, 10000); setInterval(updateCupIndicator, 5000);
}
...
// ==================== ЗАВЕРШЕНИЕ (ОРИГИНАЛ) ====================
window.addEventListener('beforeunload', () => { if (callActive) hang(false); if (voiceTimerInterval) clearInterval(voiceTimerInterval); if (voiceRecTimeout) clearTimeout(voiceRecTimeout); if (voiceStream) voiceStream.getTracks().forEach(t => t.stop()); P2PPong.destroy(); });

// ==================== ЗАПУСК (ИСПРАВЛЕННАЯ ГОНКА) ====================
initUICoreListeners(); // Сначала подписки
loadLockSettings();    // Потом запуск ядра
