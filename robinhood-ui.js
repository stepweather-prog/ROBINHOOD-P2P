...
function showIncomingControls(show) {
    const ic = document.getElementById('incoming-call-controls');
    if (ic) ic.style.display = show ? 'flex' : 'none';
}

function showActiveControls(show) {
    const ac = document.getElementById('active-call-controls');
    if (ac) ac.style.display = show ? 'flex' : 'none';
}
...
function stopRingtone() {
    if (ringtoneAudio) { ringtoneAudio.pause(); ringtoneAudio.loop = false; ringtoneAudio = null; }
}
...
function stopRingback() {
    if (ringbackAudio) { ringbackAudio.pause(); ringbackAudio.loop = false; ringbackAudio = null; }
}
...
function startVoiceTimer() {
    ...
    voiceTimerInterval = setInterval(() => {
        ...
        const m = Math.floor(voiceSeconds / 60),
            s = (voiceSeconds % 60).toString().padStart(2, '0');
        const vt = document.getElementById('voice-timer-text');
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(st => {
        ...
        voiceRecTimeout = setTimeout(() => {
            if (voiceRecorder?.state === 'recording') {
                voiceRecorder.stop();
                rMsg('⏰ Максимальная длина записи — 2 минуты', 3000);
            }
        }, MAX_RECORD_SEC * 1000);
    }).catch(e => {
...
function showChatForChannel(channelId) {
    ...
    if (ct) {
        activePeerId = ct.peerId;
        document.getElementById('robin-bar-sender').textContent = ct.name || 'Лучник';
    }
    const box = document.getElementById('chat-box');
...
function saveContacts() {
    try { localStorage.setItem('rh_contacts', JSON.stringify(contacts)); } catch (e) {}
}
...
function updateCupIndicator() {
    ...
    const be = document.getElementById('cup-blobs');
    if (be) { be.textContent = bc + '/10'; be.className = bc >= 10 ? 'full' : bc >= 7 ? 'ok' : ''; }
    const totalSec = Math.max(0, Math.round((ch.expires - Date.now()) / 1000));
    const min = Math.floor(totalSec / 60);
    const sec = totalSec % 60;
    const te = document.getElementById('cup-timer');
    if (te) { te.textContent = min + ':' + sec.toString().padStart(2, '0'); te.className = min <= 2 ? 'low' : min <= 5 ? 'ok' : ''; }
}
...
async function saveBgToDB(dataUrl) {
    try {
        const db = await openPPDatabase();
        if (!db || !db.objectStoreNames.contains('bg')) return;
        db.transaction('bg', 'readwrite').objectStore('bg').put({ k: 'custom_bg', v: dataUrl });
    } catch (e) {}
}
...
async function getBgFromDB() {
    try {
        const db = await openPPDatabase();
        if (!db || !db.objectStoreNames.contains('bg')) return null;
        return await new Promise(res => {
            const q = db.transaction('bg', 'readonly').objectStore('bg').get('custom_bg');
            q.onsuccess = () => res(q.result?.v || null);
            q.onerror = () => res(null);
        });
    } catch (e) { return null; }
}
...
async function applyCustomBg(dataUrl) {
    if (dataUrl) {
        ...
        const bs = document.getElementById('bg-status');
        if (bs) bs.textContent = 'Задан';
    } else {
        document.body.classList.remove('custom-bg');
        document.body.style.removeProperty('--custom-bg');
        const bs = document.getElementById('bg-status');
        if (bs) bs.textContent = 'Не задан';
    }
}
...
async function pbkdf2Hash(text, salt) {
    try {
        ...
        const h = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(text + salt));
...
function addPinDigit(d) {
    ...
    pinInput += d;
    updatePinDots();
    if (pinInput.length === 5) setTimeout(() => verifyPin(), 200);
}
...
async function verifyPin() {
    if (isSettingLock) {
        if (!lockPinHash) {
            lockPinHash = pinInput;
            pinInput = '';
            updatePinDots();
            if (document.getElementById('lock-subtitle')) document.getElementById('lock-subtitle').textContent = 'Повторите пин-код';
            return;
        }
        if (pinInput === lockPinHash) {
            ...
            if (ls) ls.textContent = 'Пин-код';
            unlockApp();
        } else {
            const err = document.getElementById('lock-error');
            if (err) err.textContent = 'Не совпадают. Попробуйте снова.';
            lockPinHash = '';
            pinInput = '';
            updatePinDots();
            const sub = document.getElementById('lock-subtitle');
            if (sub) sub.textContent = 'Задайте пин-код (5 цифр)';
        }
    } else {
        const hash = await pbkdf2Hash(pinInput, 'robinhood_lock_salt');
        if (hash === lockPinHash) {
            ...
            if (failedAttempts >= 3) {
                ...
                const err = document.getElementById('lock-error');
                if (err) err.textContent = 'Заблокировано. Обновите страницу.';
            } else {
                const err = document.getElementById('lock-error');
                if (err) err.textContent = 'Неверно. Осталось попыток: ' + (3 - failedAttempts);
            }
        }
    }
}
    ...
    pc.onicecandidate = e => {
        if (e.candidate) { iceBuffer.push(e.candidate); } else {
            iceFlushTimer = setTimeout(() => {
                iceBuffer.forEach(c => sendWebRTCMsg('webrtc-ice', JSON.stringify(c)));
                iceBuffer = [];
            }, 300);
        }
    };
        ...
        if (pc.connectionState == 'disconnected' && callActive && !iceRestartInProgress) {
            ...
            iceRestartTimer = setTimeout(async () => {
                if (pc && pc.connectionState === 'disconnected') {
                    try { await restartICE(); } catch (e) { hang(false); }
...
async function restartICE() {
    ...
    sendWebRTCMsg('webrtc-offer', JSON.stringify(pc.localDescription));
}

async function sendWebRTCMsg(type, sdp) {
    if (!activeChannelId) return;
    await P2PPong.sendMessage(activeChannelId, JSON.stringify({ webrtc: type, sdp }));
}
...
async function startCall() {
    ...
    try {
        ...
        setTimeout(() => { sendWebRTCMsg('webrtc-offer', JSON.stringify(o)); }, 800);
    } catch (e) { hang(false); }
    updateCallButtonState();
}
...
function initUI() {
    ...
    P2PPong.on('voice-message', (data) => {
        ...
        const avatar = ct?.avatar || '001';
        if (data.channelId === activeChannelId) {
            appendMessage(nick, '🎤 Голосовое', avatar, data.data, 'audio/webm');
        } else {
    ...
    P2PPong.on('webrtc-signal', (data) => {
        if (data.data.webrtc === 'webrtc-offer') {
            ...
            iceBuffer = [];
            try { incomingOffer = JSON.parse(data.data.sdp); } catch (e) { return; }
        ...
        if (data.data.webrtc === 'webrtc-ice' && pc) {
            try {
                const c = JSON.parse(data.data.sdp);
                if (pc.remoteDescription) pc.addIceCandidate(new RTCIceCandidate(c)).catch(er => {});
                else iceBuffer.push(c);
            } catch (e) {}
        }
    ...
    P2PPong.on('error', (data) => {
        rMsg('❌ ' + data.message, 5000);
    });
...
function initApp() {
    ...
    document.getElementById('arrow-modal')?.addEventListener('click', function(e) {
    ...
    document.getElementById('setting-bg')?.addEventListener('click', () => {
        ...
        inp.onchange = async ev => {
            ...
            r.onload = async ev2 => {
                await saveBgToDB(ev2.target.result);
                await applyCustomBg(ev2.target.result);
                rMsg('🖼 Фон обновлён!', 3000);
            };
            r.readAsDataURL(f);
        };
        inp.click();
