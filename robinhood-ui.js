...
function playSound(f) { if (!toggleSoundState) return; if (!audioPool[f]) { audioPool[f] = new Audio('assets/sounds/' + f); audioPool[f].volume = 0.5; } const a = audioPool[f]; a.currentTime = 0; a.play().catch(e => {}); }
...
function generateRandomTheme() { const hue = Math.floor(Math.random() * 360), sat = 40 + Math.floor(Math.random() * 50), bgLight = 5 + Math.floor(Math.random() * 15), bgDark = 2 + Math.floor(Math.random() * 8), id = 'random_' + Date.now(); const s = `[data-theme="${id}"]{...}`; let el = document.getElementById('gen-theme'); if (!el) { el = document.createElement('style'); el.id = 'gen-theme'; document.head.appendChild(el); } el.textContent = s; document.documentElement.setAttribute('data-theme', id); const tn = document.getElementById('theme-name'); if (tn) tn.textContent = 'Авто'; try { localStorage.setItem('robinhood_theme', id); } catch (e) {} }
...
function initUI() {
    ...
    P2PPong.on('message-received', (data) => { const ct = contacts.find(c => c.channelId === data.channelId); const nick = ct?.name || 'Лучник'; const avatar = ct?.avatar || '001'; if (data.channelId === activeChannelId) { appendMessage(nick, data.text, avatar); } else { rMsg('Новое от ' + nick, 3000); } updateCupIndicator(); updateRatchetIndicator(); updateAIStats(); playSound('arrow_hit.wav'); });
...
function initApp() {
    ...
    document.getElementById('btn-craft')?.addEventListener('click', async () => {
        try { const peerId = await P2PPong.craftArrow(); const display = document.getElementById('craft-peer-id-display'); if (display) display.textContent = peerId; document.getElementById('craft-modal')?.classList.add('active'); } catch(e) { console.error('craftArrow error:', e); }
    });
    ...
    document.getElementById('btn-verify-confirm')?.addEventListener('click', async () => {
        ...
        if (input === expected) {
            ...
            if (errEl) { errEl.textContent = '❌ Не совпадают. Попробуй снова.'; errEl.style.display = 'block'; }
        }
    });
    ...
    document.getElementById('btn-arrow')?.addEventListener('click', () => {
        if (P2PPong._peerId) {
            document.getElementById('verify-instruction').textContent = 'Введи эмодзи которые назвал пир Б:';
